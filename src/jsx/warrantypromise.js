/* Thing > Intangible > StructuredValue > WarrantyPromise - A structured value representing the duration and scope of services that will be provided to a customer free of charge in case of a defect or malfunction of a product.. Generated automatically by the reactGenerator. */  var WarrantyPromise= React.createClass({
    ...
    render: function(){
      ...
      if( props.warrantyScope ){
        if( props.warrantyScope instanceof Array ){
          warrantyScope = [ (<div key='header' data-advice='HTML for the *head* of the section'></div>) ]
          warrantyScope = warrantyScope.concat( props.warrantyScope.map( function(result, index){
              return ( <WarrantyScope {...result} key={index} /> )
           }) );
         warrantyScope.push( ( <div key='footer' data-advice='HTML for the *footer* of the section'></div> ) );
        } else {
          warrantyScope = ( <WarrantyScope props={ props.warrantyScope } /> );        }
      }
      var image;
      if( props.image ){
...
ImageObject.'></div> );
      ...
      if( props.durationOfWarranty ){
        if( props.durationOfWarranty instanceof Array ){
          durationOfWarranty = [ (<div key='header' data-advice='HTML for the *head* of the section'></div>) ]
         ...
         durationOfWarranty.push( ( <div key='footer' data-advice='HTML for the *footer* of the section'></div> ) );
        } else {
          durationOfWarranty = ( <QuantitativeValue props={ props.durationOfWarranty } /> );        }
      }
