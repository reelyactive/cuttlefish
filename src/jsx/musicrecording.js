/* Thing > CreativeWork > MusicRecording - A music recording (track), usually a single song.. Generated automatically by the reactGenerator. */
 define(['../bower_components/react/react', './alignmentobject', './videoobject', './mediaobject', './offer', './imageobject', './aggregaterating', './integer', './creativework', './review', './audioobject', './language', './musicplaylist', './person', './publicationevent', './musicalbum', './action', './organization', './comment', './thing', './audience', './musiccomposition', './boolean', './musicgroup', './duration', './place', './event'], function(React, AlignmentObject, VideoObject, MediaObject, Offer, ImageObject, AggregateRating, Integer, CreativeWork, Review, AudioObject, Language, MusicPlaylist, Person, PublicationEvent, MusicAlbum, Action, Organization, Comment, Thing, Audience, MusicComposition, Boolean, MusicGroup, Duration, Place, Event){
...
Organization.'></div> );
      ...
      if( props.inPlaylist ){
        if( props.inPlaylist instanceof Array ){
          inPlaylist = [ (<div key='header' data-advice='HTML for the *head* of the section'></div>) ]
          inPlaylist = inPlaylist.concat( props.inPlaylist.map( function(result, index){
              return ( <MusicPlaylist {...result} key={index} /> )
           }) );
         inPlaylist.push( ( <div key='footer' data-advice='HTML for the *footer* of the section'></div> ) );
        } else {
          inPlaylist = ( <MusicPlaylist props={ props.inPlaylist } /> );        }
      }
      var image;
      if( props.image ){
...
Organization.'></div> );
      ...
      if( props.byArtist ){
        if( props.byArtist instanceof Array ){
          byArtist = [ (<div key='header' data-advice='HTML for the *head* of the section'></div>) ]
          byArtist = byArtist.concat( props.byArtist.map( function(result, index){
              return ( <MusicGroup {...result} key={index} /> )
           }) );
         byArtist.push( ( <div key='footer' data-advice='HTML for the *footer* of the section'></div> ) );
        } else {
...
CreativeWork.'></div> );
      ...
      if( props.recordingOf ){
        if( props.recordingOf instanceof Array ){
          recordingOf = [ (<div key='header' data-advice='HTML for the *head* of the section'></div>) ]
          recordingOf = recordingOf.concat( props.recordingOf.map( function(result, index){
              return ( <MusicComposition {...result} key={index} /> )
           }) );
         recordingOf.push( ( <div key='footer' data-advice='HTML for the *footer* of the section'></div> ) );
        } else {
          recordingOf = ( <MusicComposition props={ props.recordingOf } /> );        }
      }
      var description;
...
Organization.'></div> );
      ...
      if( props.inAlbum ){
        if( props.inAlbum instanceof Array ){
          inAlbum = [ (<div key='header' data-advice='HTML for the *head* of the section'></div>) ]
          inAlbum = inAlbum.concat( props.inAlbum.map( function(result, index){
              return ( <MusicAlbum {...result} key={index} /> )
           }) );
         inAlbum.push( ( <div key='footer' data-advice='HTML for the *footer* of the section'></div> ) );
        } else {
          inAlbum = ( <MusicAlbum props={ props.inAlbum } /> );        }
      }
      var position;
      if( props.position ){
...
Text.'></div> );
      ...
      if( props.isrcCode ){
        if( props.isrcCode instanceof Array ){
          isrcCode = [ (<div key='header' data-advice='HTML for the *head* of the section'></div>) ]
          isrcCode = isrcCode.concat( props.isrcCode.map( function(result, index){
              return ( <div key={index} data-advice='Put your HTML here. isrcCode is a Text.'></div> )
           }) );
         isrcCode.push( ( <div key='footer' data-advice='HTML for the *footer* of the section'></div> ) );
        } else {
            isrcCode = ( <div data-advice='Put your HTML here. isrcCode is a Text.'></div> );
        }
      }
      var educationalUse;
...
Text.'></div> );
      ...
      return (<div title='MusicRecording' className='MusicRecording entity'>
        { comment }
        { copyrightYear }
        { timeRequired }
        { producer }
