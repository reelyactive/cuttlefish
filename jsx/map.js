...
CreativeWork.'></div> );
      ...
      if( props.mapType ){
        if( props.mapType instanceof Array ){
          mapType = [ (<div key='header' data-advice='HTML for the *head* of the section'></div>) ]
          mapType = mapType.concat( props.mapType.map( function(result, index){
              return ( <MapCategoryType {...result} key={index} /> )
           }) );
         mapType.push( ( <div key='footer' data-advice='HTML for the *footer* of the section'></div> ) );
        } else {
          mapType = ( <MapCategoryType props={ props.mapType } /> );        }
      }
      var provider;
      if( props.provider ){
...
Text.'></div> );
      ...
      return (<div title='Map' className='Map entity'>
        { comment }
        { copyrightYear }
        { version }
        { producer }
